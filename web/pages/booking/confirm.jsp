<%--
    BOOKING STEP 5 - Confirm booking / apply voucher
    GET /booking/confirm -> controller.BookingController#showConfirm -> BookingService#buildDraftView.
    POST action=applyVoucher recalculates VoucherQuote; POST confirm calls
    BookingService#createPendingBooking, then BookingDAO creates BOOKINGS + BOOKING_SEATS
    in one transaction and redirects to /my-booking?id=... .
    IMPORTANT: draft and voucher quote are session data, but price/seat availability are
    recomputed server-side. Never use amounts rendered in HTML as trusted values.
--%>
...
<html lang="vi">
    ...
    <!--                            <div><span class="k">Ghế</span> <c:out value="${draftView.seatLabels}"/> (${draftView.seatCount} ghế)</div>
                                ...
                                <div>
                                    <span class="k">Ghế</span>
                                    <c:out value="${draftView.seatLabels}"/>
                                    (${draftView.seatCount} ghế)
                                </div>

                                <div>
                                    <span class="k">Tổng tiền ghế</span>
                                    ${draftView.seatSubtotalLabel}
                                </div>

                                <div>
                                    <span class="k">Ưu đãi mua 5 tặng 1</span>
                                    <c:choose>
                                        <c:when test="${draftView.buyFiveDiscount > 0}">
                                            -${draftView.buyFiveDiscountLabel}
                                            (${draftView.freeTicketCount} vé miễn phí)
                                        </c:when>
                                        <c:otherwise>
                                            0 đ
                                        </c:otherwise>
                                ...
                                <div>
                                    <span class="k">Voucher</span>
                                    <c:choose>
                                        <c:when test="${draftView.voucherDiscount > 0}">
                                            -${draftView.voucherDiscountLabel}
                                        </c:when>
                                        <c:otherwise>
                                            0 đ
                                        </c:otherwise>
                            ...
                            <%-- Voucher post only recalculates session VoucherQuote; it must not create a booking. --%>
