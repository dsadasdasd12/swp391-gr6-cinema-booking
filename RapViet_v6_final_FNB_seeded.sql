...
-- Updated: REVIEWS supports half-star ratings from 0.5 to 5.0.
...
IF OBJECT_ID('dbo.BRANCH_FNB_COMBOS',    'U') IS NOT NULL DROP TABLE dbo.BRANCH_FNB_COMBOS;
IF OBJECT_ID('dbo.BRANCH_FNB_INVENTORY', 'U') IS NOT NULL DROP TABLE dbo.BRANCH_FNB_INVENTORY;
IF OBJECT_ID('dbo.FNB_COMBO_ITEMS',       'U') IS NOT NULL DROP TABLE dbo.FNB_COMBO_ITEMS;
IF OBJECT_ID('dbo.FNB_COMBOS',            'U') IS NOT NULL DROP TABLE dbo.FNB_COMBOS;
IF OBJECT_ID('dbo.FNB_PRODUCTS',          'U') IS NOT NULL DROP TABLE dbo.FNB_PRODUCTS;
IF OBJECT_ID('dbo.FNB_CATEGORIES',        'U') IS NOT NULL DROP TABLE dbo.FNB_CATEGORIES;
...
-- ============================================================
-- GROUP 7 — F&B
-- Chỉ chứa dữ liệu danh mục, món lẻ, combo và tồn kho theo chi nhánh.
-- ============================================================

CREATE TABLE dbo.FNB_CATEGORIES (
    id              INT IDENTITY(1,1) NOT NULL,
    name            NVARCHAR(100) NOT NULL,
    description     NVARCHAR(300) NULL,
    status          VARCHAR(10) NOT NULL
        CONSTRAINT DF_FNB_CATEGORIES_status DEFAULT 'ACTIVE',
    last_update     DATETIME2 NOT NULL
        CONSTRAINT DF_FNB_CATEGORIES_update DEFAULT GETDATE(),

    CONSTRAINT PK_FNB_CATEGORIES
        PRIMARY KEY (id),

    CONSTRAINT UQ_FNB_CATEGORIES_name
        UNIQUE (name),

    CONSTRAINT CK_FNB_CATEGORIES_status
        CHECK (status IN ('ACTIVE', 'INACTIVE'))
);
GO

CREATE TABLE dbo.FNB_PRODUCTS (
    id                  INT IDENTITY(1,1) NOT NULL,
    category_id         INT NOT NULL,
    name                NVARCHAR(150) NOT NULL,
    description         NVARCHAR(500) NULL,
    product_type        VARCHAR(20) NOT NULL
        CONSTRAINT DF_FNB_PRODUCTS_type DEFAULT 'ITEM',
    selling_price       DECIMAL(12,2) NOT NULL,
    image_url           VARCHAR(500) NULL,
    allowed_to_sell     BIT NOT NULL
        CONSTRAINT DF_FNB_PRODUCTS_allowed DEFAULT 1,
    status              VARCHAR(10) NOT NULL
        CONSTRAINT DF_FNB_PRODUCTS_status DEFAULT 'ACTIVE',
    last_update         DATETIME2 NOT NULL
        CONSTRAINT DF_FNB_PRODUCTS_update DEFAULT GETDATE(),

    CONSTRAINT PK_FNB_PRODUCTS
        PRIMARY KEY (id),

    CONSTRAINT UQ_FNB_PRODUCTS_name
        UNIQUE (name),

    CONSTRAINT FK_FNB_PRODUCTS_category
        FOREIGN KEY (category_id)
        REFERENCES dbo.FNB_CATEGORIES(id),

    CONSTRAINT CK_FNB_PRODUCTS_type
        CHECK (product_type IN ('ITEM')),

    CONSTRAINT CK_FNB_PRODUCTS_price
        CHECK (selling_price >= 0),

    CONSTRAINT CK_FNB_PRODUCTS_status
        CHECK (status IN ('ACTIVE', 'INACTIVE'))
);
GO

CREATE TABLE dbo.FNB_COMBOS (
    id                  INT IDENTITY(1,1) NOT NULL,
    name                NVARCHAR(150) NOT NULL,
    description         NVARCHAR(500) NULL,
    selling_price       DECIMAL(12,2) NOT NULL,
    image_url           VARCHAR(500) NULL,
    allowed_to_sell     BIT NOT NULL
        CONSTRAINT DF_FNB_COMBOS_allowed DEFAULT 1,
    status              VARCHAR(10) NOT NULL
        CONSTRAINT DF_FNB_COMBOS_status DEFAULT 'ACTIVE',
    last_update         DATETIME2 NOT NULL
        CONSTRAINT DF_FNB_COMBOS_update DEFAULT GETDATE(),

    CONSTRAINT PK_FNB_COMBOS
        PRIMARY KEY (id),

    CONSTRAINT UQ_FNB_COMBOS_name
        UNIQUE (name),

    CONSTRAINT CK_FNB_COMBOS_price
        CHECK (selling_price >= 0),

    CONSTRAINT CK_FNB_COMBOS_status
        CHECK (status IN ('ACTIVE', 'INACTIVE'))
);
GO

CREATE TABLE dbo.FNB_COMBO_ITEMS (
    combo_id       INT NOT NULL,
    product_id     INT NOT NULL,
    quantity       INT NOT NULL,

    CONSTRAINT PK_FNB_COMBO_ITEMS
        PRIMARY KEY (combo_id, product_id),

    CONSTRAINT FK_FNB_COMBO_ITEMS_combo
        FOREIGN KEY (combo_id)
        REFERENCES dbo.FNB_COMBOS(id),

    CONSTRAINT FK_FNB_COMBO_ITEMS_product
        FOREIGN KEY (product_id)
        REFERENCES dbo.FNB_PRODUCTS(id),

    CONSTRAINT CK_FNB_COMBO_ITEMS_quantity
        CHECK (quantity > 0)
);
GO

CREATE TABLE dbo.BRANCH_FNB_INVENTORY (
    branch_id           INT NOT NULL,
    product_id          INT NOT NULL,
    stock_quantity      INT NOT NULL
        CONSTRAINT DF_BFI_stock DEFAULT 0,
    enabled_at_branch   BIT NOT NULL
        CONSTRAINT DF_BFI_enabled DEFAULT 0,
    last_update         DATETIME2 NOT NULL
        CONSTRAINT DF_BFI_update DEFAULT GETDATE(),

    CONSTRAINT PK_BRANCH_FNB_INVENTORY
        PRIMARY KEY (branch_id, product_id),

    CONSTRAINT FK_BFI_branch
        FOREIGN KEY (branch_id)
        REFERENCES dbo.BRANCHES(id),

    CONSTRAINT FK_BFI_product
        FOREIGN KEY (product_id)
        REFERENCES dbo.FNB_PRODUCTS(id),

    CONSTRAINT CK_BFI_stock
        CHECK (stock_quantity >= 0)
);
GO

CREATE TABLE dbo.BRANCH_FNB_COMBOS (
    branch_id           INT NOT NULL,
    combo_id            INT NOT NULL,
    enabled_at_branch   BIT NOT NULL
        CONSTRAINT DF_BFC_enabled DEFAULT 0,
    last_update         DATETIME2 NOT NULL
        CONSTRAINT DF_BFC_update DEFAULT GETDATE(),

    CONSTRAINT PK_BRANCH_FNB_COMBOS
        PRIMARY KEY (branch_id, combo_id),

    CONSTRAINT FK_BFC_branch
        FOREIGN KEY (branch_id)
        REFERENCES dbo.BRANCHES(id),

    CONSTRAINT FK_BFC_combo
        FOREIGN KEY (combo_id)
        REFERENCES dbo.FNB_COMBOS(id)
);
GO

CREATE INDEX IX_BFI_branch_enabled
    ON dbo.BRANCH_FNB_INVENTORY(branch_id, enabled_at_branch, product_id);

CREATE INDEX IX_BFC_branch_enabled
    ON dbo.BRANCH_FNB_COMBOS(branch_id, enabled_at_branch, combo_id);
GO
...
CREATE TABLE dbo.BOOKING_FNB (
    ...
    CONSTRAINT FK_BFNB_product FOREIGN KEY (product_id) REFERENCES dbo.FNB_PRODUCTS(id),
    CONSTRAINT FK_BFNB_combo FOREIGN KEY (combo_id) REFERENCES dbo.FNB_COMBOS(id),
    CONSTRAINT CK_BFNB_type CHECK (item_type IN ('PRODUCT','COMBO')),
    CONSTRAINT CK_BFNB_target CHECK ((item_type='PRODUCT' AND product_id IS NOT NULL AND combo_id IS NULL) OR (item_type='COMBO' AND combo_id IS NOT NULL AND product_id IS NULL)),
    CONSTRAINT CK_BFNB_qty CHECK (quantity > 0),
    CONSTRAINT CK_BFNB_price CHECK (unit_price >= 0),
    CONSTRAINT CK_BFNB_status CHECK (status IN ('PENDING','PREPARING','READY','DELIVERED','CANCELLED')),
    CONSTRAINT UQ_BFNB_line UNIQUE (booking_id,item_type,product_id,combo_id)
);
CREATE INDEX IX_BOOKING_FNB_booking ON dbo.BOOKING_FNB(booking_id);
...
-- ============================================================
-- F&B DEMO SEED
-- Chỉ seed danh mục, món lẻ, combo và tồn kho theo chi nhánh.
-- ============================================================

MERGE dbo.FNB_CATEGORIES AS target
USING (
    VALUES
        (N'Bắp rang', N'Các loại bắp rang dùng tại rạp'),
        (N'Nước uống', N'Nước ngọt và nước suối đóng chai'),
        (N'Snack', N'Đồ ăn nhẹ dùng trong lúc xem phim')
...
WHEN NOT MATCHED THEN
    ...
    VALUES (source.name, source.description, 'ACTIVE');
GO

DECLARE
    @PopcornCategoryId INT = (
        SELECT id FROM dbo.FNB_CATEGORIES WHERE name = N'Bắp rang'
    ),
    @DrinkCategoryId INT = (
        SELECT id FROM dbo.FNB_CATEGORIES WHERE name = N'Nước uống'
    ),
    @SnackCategoryId INT = (
        SELECT id FROM dbo.FNB_CATEGORIES WHERE name = N'Snack'
    );

MERGE dbo.FNB_PRODUCTS AS target
USING (
    VALUES
        (@PopcornCategoryId, N'Bắp Rang Lớn', N'Bắp rang cỡ lớn', 65000.00,
         'https://images.unsplash.com/photo-1585647347483-22b66260dfff?auto=format&fit=crop&w=800&q=80'),
        (@PopcornCategoryId, N'Bắp Rang Nhỏ', N'Bắp rang cỡ nhỏ', 45000.00,
         'https://images.unsplash.com/photo-1585647347483-22b66260dfff?auto=format&fit=crop&w=800&q=80'),
        (@DrinkCategoryId, N'7Up', N'Nước ngọt 7Up', 30000.00,
         'https://images.unsplash.com/photo-1622483767028-3f66f32aef97?auto=format&fit=crop&w=800&q=80'),
        (@DrinkCategoryId, N'Aquafina', N'Nước suối Aquafina', 20000.00,
         'https://images.unsplash.com/photo-1602143407151-7111542de6e8?auto=format&fit=crop&w=800&q=80'),
        (@DrinkCategoryId, N'Pepsi', N'Nước ngọt Pepsi', 30000.00,
         'https://images.unsplash.com/photo-1629203851122-3726ecdf080e?auto=format&fit=crop&w=800&q=80'),
        (@SnackCategoryId, N'Khoai Tây', N'Khoai tây chiên giòn', 40000.00,
         'https://images.unsplash.com/photo-1566478989037-eec170784d0b?auto=format&fit=crop&w=800&q=80'),
        (@SnackCategoryId, N'Xúc Xích', N'Xúc xích nóng', 35000.00,
         'https://images.unsplash.com/photo-1612392062631-94dd858cba88?auto=format&fit=crop&w=800&q=80')
) AS source(category_id, name, description, selling_price, image_url)
ON target.name = source.name
WHEN MATCHED THEN
    UPDATE SET
        target.category_id = source.category_id,
        target.description = source.description,
        target.product_type = 'ITEM',
        target.selling_price = source.selling_price,
        target.image_url = source.image_url,
        target.allowed_to_sell = 1,
        target.status = 'ACTIVE',
        target.last_update = GETDATE()
WHEN NOT MATCHED THEN
    INSERT (
...
MERGE dbo.FNB_COMBOS AS target
USING (
    VALUES
        (
            N'Combo Solo',
            N'1 bắp rang nhỏ và 1 Pepsi',
            65000.00,
            'https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?auto=format&fit=crop&w=800&q=80'
        ),
        (
            N'Combo Couple',
            N'1 bắp rang lớn và 2 Pepsi',
            115000.00,
            'https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?auto=format&fit=crop&w=800&q=80'
        ),
        (
            N'Combo Family',
            N'2 bắp rang lớn, 3 Pepsi và 1 khoai tây',
            230000.00,
            'https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?auto=format&fit=crop&w=800&q=80'
        )
) AS source(name, description, selling_price, image_url)
ON target.name = source.name
WHEN MATCHED THEN
    UPDATE SET
        target.description = source.description,
...
;WITH ComboSeed AS (
    ...
    FROM (
        VALUES
            (N'Combo Solo', N'Bắp Rang Nhỏ', 1),
            (N'Combo Solo', N'Pepsi', 1),

            (N'Combo Couple', N'Bắp Rang Lớn', 1),
            (N'Combo Couple', N'Pepsi', 2),

            (N'Combo Family', N'Bắp Rang Lớn', 2),
            (N'Combo Family', N'Pepsi', 3),
            (N'Combo Family', N'Khoai Tây', 1)
    ) AS seed(combo_name, product_name, quantity)
    INNER JOIN dbo.FNB_COMBOS c
        ON c.name = seed.combo_name
    INNER JOIN dbo.FNB_PRODUCTS p
        ON p.name = seed.product_name
)
MERGE dbo.FNB_COMBO_ITEMS AS target
USING ComboSeed AS source
ON target.combo_id = source.combo_id
AND target.product_id = source.product_id
WHEN MATCHED THEN
    UPDATE SET target.quantity = source.quantity
WHEN NOT MATCHED THEN
    INSERT (combo_id, product_id, quantity)
    VALUES (source.combo_id, source.product_id, source.quantity);
GO

MERGE dbo.BRANCH_FNB_INVENTORY AS target
USING (
    SELECT
        b.id AS branch_id,
        p.id AS product_id,
        CAST(
            CASE p.name
                WHEN N'Bắp Rang Lớn' THEN 120
                WHEN N'Bắp Rang Nhỏ' THEN 150
                WHEN N'7Up' THEN 100
                WHEN N'Aquafina' THEN 180
                WHEN N'Pepsi' THEN 200
                WHEN N'Khoai Tây' THEN 80
                WHEN N'Xúc Xích' THEN 70
                ELSE 50
            END AS INT
        ) AS stock_quantity
    FROM dbo.BRANCHES b
    CROSS JOIN dbo.FNB_PRODUCTS p
) AS source
...
MERGE dbo.BRANCH_FNB_COMBOS AS target
USING (
    SELECT
        b.id AS branch_id,
        c.id AS combo_id
    FROM dbo.BRANCHES b
    CROSS JOIN dbo.FNB_COMBOS c
) AS source
ON target.branch_id = source.branch_id
AND target.combo_id = source.combo_id
WHEN MATCHED THEN
    UPDATE SET
        target.enabled_at_branch = 1,
        target.last_update = GETDATE()
