...
/** Owns validation and mutation rules for a manager's seat-map edits. */
public class SeatLayoutService {
    private final SeatService seatService = new SeatService();
    private final ShowtimeDAO showtimeDAO = new ShowtimeDAO();

    public String apply(int hallId, String action, Map<String, String> values) {
        if (hallId <= 0) throw new IllegalArgumentException("Phòng chiếu không hợp lệ.");
        ...
        if ("clearAll".equals(action)) {
            if (!seatService.deleteSeatsOfHall(hallId)) throw new IllegalArgumentException("Không thể xóa sơ đồ ghế.");
            return "Đã xóa toàn bộ sơ đồ ghế.";
        }
        if ("bulkAdd".equals(action)) return bulkAdd(hallId, values);
    ...
    private String bulkAdd(int hallId, Map<String, String> values) {
        ...
        if ("row".equals(kind)) {
            ...
            for (int i = 1; i <= count; i++) if (seatService.insertSeat(hallId, row, i, type, maintenance)) added++;
            ...
            for (char r : rows(value(values, "bulkRowStart"), value(values, "bulkRowEnd"))) {
                if (seatService.insertSeat(hallId, String.valueOf(r), column, type, maintenance)) added++;
            }
        } else if ("grid".equals(kind)) {
            int count = seatNumber(value(values, "bulkSeatCount"));
            for (char r : rows(value(values, "bulkRowStart"), value(values, "bulkRowEnd"))) {
                for (int i = 1; i <= count; i++) if (seatService.insertSeat(hallId, String.valueOf(r), i, type, maintenance)) added++;
            }
        } else {
    ...
    private String changeOne(int hallId, String action, Map<String, String> values) {
        ...
        if (!code.matches("^[A-Z]\\d+$")) throw new IllegalArgumentException("Mã ghế không hợp lệ.");
        ...
        boolean changed;
        if ("add".equals(action)) changed = seatService.insertSeat(hallId, row, number, value(values, "seatType"), maintenance);
        else if ("update".equals(action)) changed = seatService.updateSeatConfig(hallId, row, number, value(values, "seatType"), maintenance);
        else if ("delete".equals(action)) changed = seatService.deleteSeat(hallId, row, number);
        else throw new IllegalArgumentException("Thao tác sơ đồ ghế không hợp lệ.");
        if (!changed) throw new IllegalArgumentException("Không thể thay đổi ghế " + code + ".");
        return "Đã cập nhật ghế " + code + ".";
    }

    private String value(Map<String, String> values, String key) {
        String value = values.get(key);
        if (value == null || value.trim().isEmpty()) throw new IllegalArgumentException("Thiếu dữ liệu cấu hình ghế.");
        return value.trim();
    }

    private String row(String value) {
        String normalized = value.trim().toUpperCase();
        if (!normalized.matches("^[A-Z]$")) throw new IllegalArgumentException("Hàng ghế phải là một chữ cái.");
        return normalized;
    }

    private char[] rows(String start, String end) {
        char from = row(start).charAt(0), to = row(end).charAt(0);
        if (from > to) throw new IllegalArgumentException("Hàng bắt đầu phải đứng trước hàng kết thúc.");
        char[] result = new char[to - from + 1];
        for (int i = 0; i < result.length; i++) result[i] = (char) (from + i);
        return result;
    }
