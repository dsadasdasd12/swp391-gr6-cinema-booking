    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (conn != null) {
            try {
                ...
                try (PreparedStatement ps = conn.prepareStatement(sqlMovies); ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        activeMovies = rs.getInt(1);
                    }
                }

                // Customer count
                String sqlCust = "SELECT COUNT(*) FROM dbo.[USER] WHERE role = 'CUSTOMER'";
                try (PreparedStatement ps = conn.prepareStatement(sqlCust); ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(sqlCom); ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
